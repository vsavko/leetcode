...
public List<List<Integer>> fourSum(int[] num, int target) {
    ...
    Map<Integer, List<int[]>> twoSumMap = new HashMap<>(); // for holding visited pair sums. 
        All pairs with the same pair sum are grouped together
    ...
    for (int i = 0; i < num.length; i++) {
        ...
        for (int j = i + 1; j < num.length; j++) {
            ...
            // for each pair sum, check if the pair sum that is needed to get the target has 
                been visited.               
            ...
            for (int[] pair : ls) {
                // we have two pairs: one is indicated as (pair[0], pair[1]), the other is 
                    (i, j).
                ...
                int m1 = Math.min(pair[0], i);  // m1 will always be the smallest index
                    int m2 = Math.min(pair[1], j);  // m2 will be one of the middle two 
                        indices
                    int m3 = Math.max(pair[0], i);  // m3 will be one of the middle two 
                        indices
                    int m4 = Math.max(pair[1], j);  // m4 will always be the largest index
                    
                    if (m1 == m3 || m1 == m4 || m2 == m3 || m2 == m4) continue;  // two 
                        pairs are overlapping, so just ignore this case
            ...
            // mark that we have visited current pair and add it to the corrsponding pair 
                sum group.
            // here we've encoded the pair indices i and j into an integer array of length 2
                .
            twoSumMap.computeIfAbsent(num[i] + num[j], key -> new ArrayList<>()).add(new 
                int[] {i, j});
        }
    }
