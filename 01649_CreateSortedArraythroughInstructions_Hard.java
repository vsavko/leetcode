...
public int createSortedArray(int[] instructions) {
        ...
        FenwickTree ft = new FenwickTree(max+1);
        int ans = 0;
        for(int i = 0; i < instructions.length; i++) {
            int num = instructions[i];
            int low = ft.sum(num-1);
            int high = i - ft.sum(num);
            ans += Math.min(ft.sum(num-1), i - ft.sum(num));
            ans %= 1000_000_007;
            ft.insert(num, 1);
        }
    ...
    class FenwickTree {
        ...
        public FenwickTree(int num) {
            //create an empty Fenwich tree array
            this.tree = new int[num+1];
        }
        
        public FenwickTree(int [] arr) {
            //create it from array
            this.tree = Arrays.copyOf(arr, arr.length+1);
